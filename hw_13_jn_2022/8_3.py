...
def f1(s1, s2, n):
    if (s1 + s2) >= 63 and (s1 + s2) <= 74:
        ...
        if n >= 2: return False
            ...
            return f1(s1 + 2, s2, n + 1) and f1(s1, s2 + 2, n + 1) and f1(s1 * 2, s2, n + 1) and f1(s1, s2 * 2, n + 1)
        else:
            return f1(s1 + 2, s2, n + 1) or f1(s1, s2 + 2, n + 1) or f1(s1 * 2, s2, n + 1) or f1(s1, s2 * 2, n + 1)


for s in range(1, 48):
    if f(15, s, 0) and not(f1(15,s,0)):
        print(s)